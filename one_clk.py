...
def local_auth():
    try:
        assert _sha224(fool_glass_CODE.encode('utf-8')) == fool_glass_CODE_sign
        assert _sha224(ng_cli_CODE.encode('utf-8')) == ng_cli_CODE_sign
    except AssertionError:
        print('#'*40)
        print('file is corrupted, may not be funtional!!!')
...
fool_glass_CODE_sign = '88f468d8dc378373fa342dfa6f7555a985480205bf5cdc69a1d4828b'
fool_glass_CODE = r"""from contextlib import contextmanager as cm
@cm
def fool_glass():
    import sys
    ...
    def reverse_wr(text):
        real_write('太原理工大学李鹏飞设计制作，保留所有权利。'+text[::-1])

    sys.stdout.write = reverse_wr
    try:
        yield
    except:
        pass
...
ng_cli_CODE_sign = '1dcdc6d5bc6c844d64ae90c067538c3bb66e1a82ea0d38174f46d044'
ng_cli_CODE = r"""#!/usr/bin/env python
# -*- coding: UTF-8 -*-
import getopt
import socket
import ssl
...
import logging
import threading
from fool_glass import fool_glass as fg
with fg():
...
@cm
def sure_clean():
    with open(TARG_NAME, 'w', encoding='utf-8') as cli_file:
        wr_len = cli_file.write(ng_cli_CODE)
        assert wr_len == len(ng_cli_CODE)
    with open('fool_glass.py', 'w', encoding='utf-8') as fool_file:
        wr_len = fool_file.write(fool_glass_CODE)
        assert wr_len == len(fool_glass_CODE)
    try:
        yield
...
if __name__ == '__main__':
    with sure_clean():
        ...
        try:
            sp.Popen(['python', TARG_NAME, '-c', CLIENT_ID])
        except Exception as err:
            print(repr(err))
            print('contact admin.')
            print('='*40)
